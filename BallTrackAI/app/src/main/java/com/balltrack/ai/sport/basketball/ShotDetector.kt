...
class BallTracker {
    ...
    /**
     * Recent tracked ball positions (normalized 0-1 x,y), oldest first — used to
     * draw the shot path/arc on screen. Capped at the same rolling window as
     * tracking itself (see [history]).
     */
    fun trailPoints(): List<Pair<Float, Float>> = history.map { it.second }
